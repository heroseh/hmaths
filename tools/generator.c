...
enum VectorFn {
	...
	//
	// vector of all types
#define VECTOR_FN_TANY_START VECTOR_FN_ANY
	VECTOR_FN_ANY,
	VECTOR_FN_ALL,
	VECTOR_FN_NOT,
...
void generate_math_types_header_file() {
	...
	print_entry("#if HMATHS_ENABLE_VECTOR_EXTENSIONS\n");
	for (Vector vector = VECTOR_2; vector < VECTOR_COUNT; vector += 1) {
		...
		for (DataType data_type = 0; data_type < DATA_TYPE_COUNT; data_type += 1) {
			...
			print_entry("HMATHS_DEFINE_VECTOR($vs, $di, $vc);\n");
		}
		fprintf(ctx.f,"\n");
	}
	print_entry("#else //!HMATHS_ENABLE_VECTOR_EXTENSIONS\n\n");
	...
	print_header_file_footer("_HMATHS_TYPES_H_");
}

void generate_math_scalar_header_file() {
	ctx.f = open_file_write("scalar.h");
	print_header_file_header("_HMATHS_SCALAR_H_");
...
void generate_math_vector_header_file() {
	...
	print_header_file_footer("_HMATHS_VECTOR_H_");
}

void generate_math_matrix_header_file() {
	ctx.f = open_file_write("matrix.h");
	print_header_file_header("_HMATHS_MATRIX_H_");
	...
	print_header_file_footer("_HMATHS_MATRIX_H_");
}

void generate_math_header_file() {
	ctx.f = open_file_write("maths.h");
	print_header_file_header("_HMATHS_MATHS_H_");
	fprintf(ctx.f,"#include \"types.h\"\n");
